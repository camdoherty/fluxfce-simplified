...
DEPENDENCY_CHECKER_SCRIPT_NAME = (
    "fluxfce_deps_check.py"  # Name of your dependency checker script
)
...
def setup_cli_logging(verbose: bool):
    ...
    if not logging.getLogger("fluxfce_core").hasHandlers():
        ...
        logging.getLogger("fluxfce_core").propagate = (
            False  # Prevent core logs going to root
        )
...
def print_status(status_data: dict):
    ...
    if status_data["config"].get("error"):
        ...
        log.info(
            f"  Location:      {status_data['config'].get('latitude', 'N/A')}, {status_data['config'].get('longitude', 'N/A')}"
        )
    ...
    if status_data["sun_times"].get("error"):
    ...
    elif status_data["sun_times"].get("sunrise") and status_data["sun_times"].get(
        "sunset"
    ):
        ...
        try:
            log.info(
                f"  Sunrise:       {sunrise_dt.isoformat(sep=' ', timespec='seconds')}"
            )
            log.info(
                f"  Sunset:        {sunset_dt.isoformat(sep=' ', timespec='seconds')}"
            )
        except (
            Exception
        ):  # Fallback if isoformat attributes are missing (e.g. raw string)
            log.info(f"  Sunrise:       {sunrise_dt}")
            log.info(f"  Sunset:        {sunset_dt}")
    else:
        log.info("  Could not be calculated (check config/location).")
    log.info(
        f"  Current Period:  {status_data.get('current_period', 'unknown').capitalize()}"
    )
    ...
    if status_data["schedule"].get("error"):
        ...
        if jobs:
            ...
            enabled
        ):  # This case might indicate scheduler thinks it's on but no jobs found
            log.info(
                "  Status:        Enabled (No specific fluxfce jobs found - state mismatch or just scheduled?)"
            )
        else:
    ...
    if status_data["systemd"].get("error"):
        ...
        log.info(
            f"  Scheduler Timer ({fluxfce_core.SCHEDULER_TIMER_NAME}): {timer_status}"
        )
        log.info(
            f"  Scheduler Service ({fluxfce_core.SCHEDULER_SERVICE_NAME}): {service_status}"
        )
        log.info(f"  Login Service ({fluxfce_core.LOGIN_SERVICE_NAME}): {login_status}")
        log.info(
            f"  Resume Service ({fluxfce_core.RESUME_SERVICE_NAME}): {resume_status}"
        )
        log.info(
...
def ask_yes_no_cli(prompt: str, default_yes: bool = False) -> bool:
    ...
    while True:
        try:
            ...
            )  # Use print for feedback to input
        except EOFError:
            print()
            return default_yes
...
""",
    ...
    subparsers.add_parser(
        "install", help="Install systemd units and enable automatic scheduling."
    )
    subparsers.add_parser(
        "uninstall",
        help="Remove systemd units & clear schedule (prompts to remove config).",
    )
    subparsers.add_parser(
        "enable", help="Enable automatic scheduling (schedules transitions)."
    )
    subparsers.add_parser(
        "disable", help="Disable automatic scheduling (clears scheduled transitions)."
    )
    subparsers.add_parser(
        "status", help="Show config, calculated times, and schedule status."
    )
    subparsers.add_parser(
        "force-day", help="Apply Day Mode settings now (disables automatic scheduling)."
    )
    subparsers.add_parser(
        "force-night",
        help="Apply Night Mode settings now (disables automatic scheduling).",
    )
    parser_set_default = subparsers.add_parser(
        "set-default",
        help="Save current desktop look as the new default for Day or Night mode.",
    )
    ...
    try:
        ...
        if args.command == "install":
            ...
            if not dependency_checker_script.exists():
                log.error(
                    f"Dependency checker script '{DEPENDENCY_CHECKER_SCRIPT_NAME}' not found in {SCRIPT_DIR}"
                )
                log.error(
                    "Please ensure it's present alongside fluxfce_cli.py. Aborting installation."
                )
                sys.exit(1)
            ...
            if process.returncode != 0:
                log.error(
                    f"Dependency check/setup script failed or reported unresolved critical issues (exit code: {process.returncode})."
                )
                log.error(
                    "Please review its output above. Aborting fluxfce installation."
                )
                sys.exit(1)
            ...
            if not config_existed_before_setup:
                ...
                if (
                    (
                        current_lat
                        == core_config.DEFAULT_CONFIG[loc_section]["LATITUDE"]
                        and current_lon
                        == core_config.DEFAULT_CONFIG[loc_section]["LONGITUDE"]
                    )
                    or not current_lat
                    or not current_lon
                ):
                    log.info(
                        "Existing configuration found, but location seems to be default or missing."
                    )
                    if ask_yes_no_cli(
                        "Do you want to run interactive setup for location and timezone?",
                        default_yes=True,
                    ):
                        run_interactive_setup = True
                    else:
                        log.info(
                            "Skipping interactive setup. Using current/default config values."
                        )
                else:
                    log.info(
                        f"Existing configuration found at {fluxfce_core.CONFIG_FILE}. Using it."
                    )
            ...
            if run_interactive_setup:
                ...
                final_tz = config.get(
                    "Location", "TIMEZONE", fallback=default_tz
                ...
                if detected_tz:
                    ...
                    if detected_tz != final_tz:
                        if ask_yes_no_cli(
                            f"Use detected timezone '{detected_tz}' (current is '{final_tz}')?",
                            default_yes=True,
                        ):
                            final_tz = detected_tz
                    else:
                        print(
                            f"Detected timezone matches current/default ('{final_tz}')."
                        )
                else:
                    print(
                    ...
                    if final_tz == default_tz:
                        print(
                            f"Consider setting it manually if '{default_tz}' is incorrect."
                        )
                ...
                print(
                    "(Format: e.g., 43.65N, 79.38W - Press Enter to use defaults from config or internal defaults)"
                )

                # Get current or internal defaults to display in prompt
                prompt_default_lat = config.get(
                    "Location",
                    "LATITUDE",
                    fallback=core_config.DEFAULT_CONFIG["Location"]["LATITUDE"],
                )
                prompt_default_lon = config.get(
                    "Location",
                    "LONGITUDE",
                    fallback=core_config.DEFAULT_CONFIG["Location"]["LONGITUDE"],
                )
                ...
                try:
                    lat_input = input(
                        f"Enter Latitude [{prompt_default_lat}]: "
                    ).strip()
                    lon_input = input(
                        f"Enter Longitude [{prompt_default_lon}]: "
                    ).strip()
                    ...
                    core_helpers.latlon_str_to_float(
                        temp_lat
                    ...
                    if chosen_lat != config.get(
                        "Location", "LATITUDE"
                    ) or chosen_lon != config.get("Location", "LONGITUDE"):
                        coords_changed = True
                ...
                if coords_changed:
                    ...
                    print(
                        f"Using coordinates: Latitude={chosen_lat}, Longitude={chosen_lon}"
                    )
                    config_needs_saving_after_interactive_setup = True
                else:
                    print(
                        f"Using coordinates: Latitude={chosen_lat}, Longitude={chosen_lon} (no changes made)."
                    )

                # -- Save Config --
                if (
                    config_needs_saving_after_interactive_setup
                    or not config_existed_before_setup
                ):
            ...
            log.info(
                f"the script ({pathlib.Path(SCRIPT_PATH).name}) or a symlink to it is in your PATH."
            )
            log.info(
            ...
            log.info("     $ echo $PATH  # Check if directory is listed")
            log.info(
                f"     # If not, add 'export PATH=\"{user_bin_dir}:$PATH\"' to your ~/.bashrc or ~/.zshrc"
            )
            log.info("     # Then run 'source ~/.bashrc' or restart your terminal.")
            log.info("  2. Make the main script executable (if not already):")
            log.info(f'     $ chmod +x "{SCRIPT_PATH}"')
            log.info(f"  3. Create a symbolic link in '{user_bin_dir}':")
            log.info(
                f"     $ ln -s -f \"{SCRIPT_PATH}\" \"{user_bin_dir / 'fluxfce'}\""
            )  # -f to overwrite if exists
            log.info("     (This allows 'fluxfce_core' to be found by the script.)")

            log.info(
                "\nAlternative (Proper Python Package Installation - Recommended for broader distribution):"
            )
            log.info(
                "  - If you cloned the repository, navigate to its root directory."
            )
            log.info("  - Create packaging files (e.g., pyproject.toml).")
            log.info(
                "  - Run 'pip install .'. This typically installs the 'fluxfce' command correctly."
            )
            ...
            config_dir_path = (
                fluxfce_core.CONFIG_DIR
            )  # Assuming CONFIG_DIR is accessible
            if config_dir_path.exists():
                log.warning(f"\nConfiguration directory found at: {config_dir_path}")
                if ask_yes_no_cli(
                    "Do you want to REMOVE this configuration directory?",
                    default_yes=False,
                ):
                    try:
                        ...
                        log.error(
                            f"Error removing configuration directory {config_dir_path}: {e}"
                        )
                else:
                    log.info("Configuration directory kept.")
            else:
                log.debug(
                    f"Configuration directory {config_dir_path} not found, nothing to remove there."
                )
            ...
            if not fluxfce_core.CONFIG_FILE.exists():
                ...
                log.error(
                    "Please run 'fluxfce install' first or ensure your config is in place."
                )
                exit_code = 1
            else:
                fluxfce_core.enable_scheduling(
            ...
            log.info(
                f"Current desktop settings saved as default for {mode.capitalize()} mode."
            )
            log.info(
                "(Run 'fluxfce enable' if you want to (re)activate scheduling with new defaults)."
            )
        ...
        log.error(
            f"FluxFCE Error: {e}", exc_info=args.verbose
        )  # Provide traceback if verbose
        ...
        )  # Always provide traceback for unexpected
