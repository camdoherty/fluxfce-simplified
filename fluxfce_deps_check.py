...
DEPS_TO_CHECK = {
    ...
    "systemctl": ("systemd", "Systemd control tool ('systemctl')"), # Should always be present
    "timedatectl": ("systemd", "Systemd time/date tool ('timedatectl')"), # Should always be present
...
XFCE4_UTILS_FALLBACK = "xfce4-session" # If xfce4-utils isn't the direct package but part of a meta
...
def run_command(command: List[str], check_exit_code: bool = True, capture_output: bool = False) -> Tuple[int, Optional[str], Optional[str]]:
    """Runs a system command."""
    try:
        process = subprocess.run(
            command,
            check=check_exit_code if not capture_output else False, # check=True with capture needs careful handling
        ...
        print_error(f"Command not found: {command[0]}. This is unexpected for core system utilities.")
        return -1, None, None
    except subprocess.CalledProcessError as e:
        stderr_msg = f": {e.stderr}" if e.stderr else ""
        print_error(f"Command '{' '.join(e.cmd)}' failed with exit code {e.returncode}{stderr_msg}")
        return e.returncode, e.stdout, e.stderr
    except Exception as e:
        print_error(f"An unexpected error occurred running command '{' '.join(command)}': {e}")
        return -2, None, None
...
def ask_yes_no(prompt: str, default_yes: bool = False) -> bool:
    ...
    while True:
        try:
            ...
            print() # Newline
            return default_yes # Or False, depending on desired non-interactive behavior
...
def check_python_version() -> bool:
    ...
    if sys.version_info >= MIN_PYTHON_VERSION:
        ...
        print_error(f"Python version is {sys.version_info.major}.{sys.version_info.minor}. "
...
def check_service_status(service_name: str, friendly_name: str) -> Tuple[bool, bool]:
    ...
    # Check active
    ret_active, _, _ = run_command(['systemctl', 'is-active', '--quiet', service_name], check_exit_code=False)
    if ret_active == 0:
        is_active = True
    ...
    # Check enabled
    ret_enabled, _, _ = run_command(['systemctl', 'is-enabled', '--quiet', service_name], check_exit_code=False)
    if ret_enabled == 0:
        is_enabled = True
...
def install_package(pkg_name: str, friendly_name: str, is_xsct_fallback: bool = False) -> bool:
    ...
    if not ask_yes_no(install_prompt):
        ...
        if pkg_name == "xsct" and not is_xsct_fallback: # Only show manual for initial xsct attempt
    ...
    ret_install, _, _ = run_command(['sudo', 'apt', 'install', '-y', pkg_name]) # -y for non-interactive install after user agrees
    if ret_install == 0:
        ...
        if pkg_name == "xsct" and not is_xsct_fallback: # Offer manual instructions if 'apt install xsct' fails
            print_info("The package 'xsct' might not be available in your current repositories or the installation failed.")
            print_manual_xsct_instructions()
        elif pkg_name == "xfce4-utils" and not is_xsct_fallback: # Try fallback for xfconf-query
            print_info(f"Trying fallback package '{XFCE4_UTILS_FALLBACK}' for XFCE utilities.")
            return install_package(XFCE4_UTILS_FALLBACK, friendly_name, is_xsct_fallback=True) # a bit of recursion
        return False

def enable_service(service_name: str, friendly_name: str) -> bool:
    """Attempts to enable and start a systemd service."""
    if not ask_yes_no(f"Attempt to enable and start {friendly_name} (service: {service_name}) using 'sudo systemctl enable --now {service_name}'?"):
        print_info(f"Skipping enabling of {friendly_name}.")
        return False

    print_info(f"Attempting to enable and start {service_name}...")
    ret_enable, _, _ = run_command(['sudo', 'systemctl', 'enable', '--now', service_name])
    if ret_enable == 0:
...
def print_manual_xsct_instructions():
    ...
    print_warning("If 'sudo apt install xsct' failed or was skipped, please follow these general steps:")
    ...
    print_info("     # OR, to install to ~/.local/bin (preferred if you don't want to use sudo):")
...
def main():
    ...
    if os.geteuid() == 0:
        print_warning("This script is not designed to be run as root, though it will invoke 'sudo' for installations.")
        if not ask_yes_no("Continue anyway?", default_yes=False):
            sys.exit(1)

    all_deps_ok = True
    missing_packages_to_install: Dict[str, str] = {} # pkg_name: friendly_name
    services_to_manage: List[Tuple[str,str,bool]] = [] # service_name, friendly_name, needs_enable
    ...
    for cmd, (pkg, friendly) in DEPS_TO_CHECK.items():
        if not check_command_installed(cmd, friendly):
            ...
            if cmd == "xsct":
                ...
                if "xsct" not in missing_packages_to_install: # Avoid duplicates if listed by another tool check
                     missing_packages_to_install = {"xsct": friendly, **missing_packages_to_install}
    ...
    for service, (pkg, friendly, enable_flag) in SERVICES_TO_CHECK.items():
        ...
        if not active or (enable_flag and not enabled):
            all_deps_ok = False
            if pkg not in missing_packages_to_install: # If service is down, package might be missing
                 missing_packages_to_install[pkg] = f"Package for {friendly}"
            if enable_flag and (not active or not enabled): # Only add to enable list if it needs enabling
    ...
    if all_deps_ok:
        ...
        print_info("Note: 'systemd' and 'python3' core presence is assumed by this script if command checks pass.")
        sys.exit(0)
    else:
    ...
    if services_to_manage:
        ...
        for service, friendly, needs_enable in services_to_manage:
            if needs_enable: # At this point, only services needing enabling should be here
        ...
        print_info("No services requiring management (or user skipped previous prompts).")
    ...
    print_info("If 'xsct' installation failed via apt, ensure you follow the manual steps if needed.")
...
if __name__ == "__main__":
    ...
    if not shutil.which("apt"):
        print_error("This script is intended for APT-based systems (Debian, Ubuntu, Mint).")
