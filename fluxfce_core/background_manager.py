...
# --- Configuration ---
# Profiles will be stored within the main fluxfce config directory
...
class BackgroundManager:
    ...
    def _get_prop(self, prop_path: str) -> str | None:
        """Queries xfconf for a single property and returns its value."""
    ...
    def _list_props(self, prop_path: str) -> list[str]:
        """Lists all properties under a given path in xfconf."""
    ...
    def _set_prop(self, prop_path: str, prop_type: str, value: Any) -> None:
        """Creates or sets an xfconf property."""
        cmd = ["xfconf-query", "-c", "xfce4-desktop", "-p", prop_path, "--create",
               "-t", prop_type, "-s", str(value)]
        helpers.run_command(cmd)

    def _set_rgba_prop(self, prop_path: str, rgba_string: str) -> None:
        """Sets an RGBA color property, which requires multiple -t/-s flags."""
        # Clean the input string to be 'r,g,b,a'
        ...
        if len(values) != 4:
            log.warning(f"Invalid number of components in RGBA string: {rgba_string}")
            return

        # xfconf-query needs the property reset before setting a new array
        helpers.run_command(["xfconf-query", "-c", "xfce4-desktop", "-p", prop_path, "-r"])
        
        cmd = ["xfconf-query", "-c", "xfce4-desktop", "-p", prop_path, "-n"] # -n for new
        for val in values:
            cmd.extend(["-t", "double", "-s", val])
        
        # Use a low-level helper to avoid error on creating existing prop
        helpers.run_command(cmd, check=False)

    def _reset_monitor_props(self, monitor_name: str) -> None:
        """Recursively resets all backdrop properties for a given monitor."""
    ...
    def _get_connected_monitors(self) -> list[str]:
        """Returns a list of names for all connected monitors via xrandr."""
    ...
    def install_default_profiles(self) -> None:
        """Writes the default day/night profiles to the filesystem."""
        ...
        try:
            ...
            log.info(f"Written default day profile to {day_profile_path}")
            ...
            log.info(f"Written default night profile to {night_profile_path}")
        except OSError as e:
            raise XfceError(f"Failed to write default profiles: {e}") from e

    def save_current_to_profile(self, profile_name: str) -> None:
        """Saves the current XFCE desktop background config to a profile file."""
        ...
        if span_prop:
            ...
            if span_image_path:
                log.info(" -> Detected 'Span screens' mode. Saving global profile.")
        ...
        log.info(" -> Saving per-monitor settings.")
        profile_blocks = []
        for monitor in self._get_connected_monitors():
            ...
            for workspace in workspaces:
                ...
                # The RGBA prop is a multi-line string of float values
                ...
                settings = {
                    ...
                    "rgba1": rgba1_str, "rgba2": rgba2_str,
                }
                ...
                if settings['image_path'] and image_style_name != "none":
                    block.extend([
                        "type=image",
                        f"image_path={settings['image_path']}",
                        f"image_style={image_style_name}"
                    ])
                else:
                    color_style_code = int(settings['color_style'] or 0)
                    color_style_name = STYLE_COLOR_MAP.get(color_style_code, "solid")
                    if color_style_name == "solid":
                        ...
                        block.extend([
                            "type=gradient",
                            f"gradient_direction={color_style_name}"
                        ])
                        if settings['rgba1']: block.append(f"color1={settings['rgba1']}")
        ...
        if profile_blocks:
            ...
            log.warning("No explicit desktop settings found to save.")
    ...
    def load_profile(self, profile_name: str) -> None:
        ...
        log.info("Resetting existing desktop settings to default...")
        for monitor in self._get_connected_monitors():
            self._reset_monitor_props(monitor)
        ...
        if settings_map.get("monitor") == "--span--":
            ...
            log.info(f"Applying per-monitor profile: {profile_name}")
            blocks = re.split(r'\n\s*\n', content.strip())
            for block in blocks:
                ...
                if settings.get("type") == "image":
                    ...
                    self._set_prop(f"{base_path}/image-style", "int", 0) # 'none'
