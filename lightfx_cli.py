...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="lightfx (CLI): Manage XFCE appearance via sunrise/sunset timing.",
        formatter_class=argparse.RawTextHelpFormatter,
        epilog="""
Examples:
  lightfx install          # Interactive setup, install units, and enable scheduling
  lightfx status -v        # Show detailed status, including profiles and services
  lightfx day              # Apply Day mode now without disabling auto switching
  lightfx enable           # Enable automatic scheduling (sets up systemd timers)
  lightfx set-default --mode day # Save current desktop look as the new Day default
  lightfx uninstall        # Remove systemd units and schedule (prompts for config removal)
""",
    )
    ...
    try:
        ...
        if args.command == "install":
            ...
            lightfx_core.install_lightfx(script_path=SCRIPT_PATH, python_executable=PYTHON_EXECUTABLE)
            ...
            lightfx_core.uninstall_lightfx()
